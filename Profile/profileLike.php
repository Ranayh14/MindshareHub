...
<body class="bg-gray-900 text-white flex">
    ...
    <div class="flex-1 flex flex-col ml-64 min-h-screen mr-4">
        ...
        <main class="p-4 flex-1 space-y-4">
            <?php if($resultLikes && mysqli_num_rows($resultLikes) > 0): ?>
                <?php while($row = mysqli_fetch_assoc($resultLikes)): ?>
                    <div class="bg-gray-800 p-4 rounded-lg">
                        <div class="flex items-center mb-2">
                            <img src="<?php echo (!empty($row['profile_picture']) ? '../Asset/' . htmlspecialchars($row['profile_picture']) : '../Asset/pp1.png'); ?>" alt="Profile" class="rounded-full w-10 h-10 object-cover mr-3" />
                            <div>
                                <span class="font-bold"><?php echo htmlspecialchars($row['username']); ?></span>
                                <span class="block text-gray-400 text-sm" data-time="<?php echo htmlspecialchars($row['created_at']); ?>">
                                    <!-- Time akan diisi oleh JavaScript -->
                                </span>
                            </div>
                        </div>
                        <p class="mb-2"><?php echo nl2br(htmlspecialchars($row['content'])); ?></p>
                        <?php if($row['image_path']): ?>
                            <img src="../Asset/<?php echo htmlspecialchars($row['image_path']); ?>" alt="Post Image" class="mb-2 max-h-64 object-cover rounded">
                        <?php endif; ?>
                ...
                <?php endwhile; ?>
            <?php else: ?>
                <p class="text-gray-500">Belum ada post yang di-like.</p>
            <?php endif; ?>
        </main>
    </div>
